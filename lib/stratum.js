...
class Stratum extends EventEmitter {
  ...
  async handleSubmit(conn, msg) {
    ...
    //TODO: difficulty is returning 0 always? Prob I'm not doing this right.
    //however client end is targeting for solo blocks rn so not a prob if we check:
    ...
    const diffTarget = common.getTarget(diffTargetBits);
    const difficulty = common.getDifficulty(share.powHash());
    
    if(share.powHash().compare(diffTarget) > 0){
    ...
    if (share.powHash().compare(job.target) <= 0) {
      ...
      //TODO: remove this crap. Here for testing. If you fail to generate the merkleRoot 
      //it'll log it here
      console.log('verify happens',share.powHash(),job.target);
      console.log('didnt verify the share vs target',share.powHash().length,job.target.length,share.verify(job.target),share.powHash().compare(job.target),job.target.toString('hex'))
    }
