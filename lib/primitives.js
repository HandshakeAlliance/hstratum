...
class Submission {
  ...
  static fromPacket(msg) {
    ...
    */ 
    ...
    assert(util.isHex(msg.params[5]), "Mask must be a string.");
...
class Job {
  ...
  check(nonce1, subm) {
    ...
    let mask = Buffer.from(subm.mask,'hex');
    this.attempt.mask = mask;
  ...
  toJSON() {
    return [
      ...
      this.attempt.witnessRoot.toString("hex"), //needed?
